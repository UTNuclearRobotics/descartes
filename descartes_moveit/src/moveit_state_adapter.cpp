                                    ...
                                    const std::string& world_frame, const std::string& tcp_frame)
{
  ...
  if (!model)
  {
    ROS_ERROR_NAMED("descartes", "Failed to load robot model from robot description parameter: %s", robot_description.c_str());
    return false;
  }
                                    ...
                                    const std::string &world_frame, const std::string &tcp_frame)
{
  ...
  if (!joint_group_)
  {
    ROS_ERROR_NAMED("descartes", "%s: Joint group '%s' does not exist in robot model", __FUNCTION__, group_name_.c_str());
    std::stringstream msg;
    msg << "Possible group names: " << robot_state_->getRobotModel()->getJointModelGroupNames();
    ROS_ERROR_NAMED("descartes", msg.str().c_str());
    return false;
  }
  ...
  {
    ROS_ERROR_NAMED("descartes", "%s: Tool frame '%s' does not match group tool frame '%s', functionality"
  ...
  {
    ROS_WARN_NAMED("descartes", "%s: Could not determine velocity limits of RobotModel from MoveIt", __FUNCTION__);
  }

  if (seed_states_.empty())
  {
    seed_states_ = seed::findRandomSeeds(*robot_state_, group_name_, SAMPLE_ITERATIONS);
    ROS_DEBUG_NAMED("descartes", "Generated %lu random seeds", static_cast<unsigned long>(seed_states_.size()));
  }

  auto model_frame = robot_state_->getRobotModel()->getModelFrame();
  if (world_frame_ != model_frame)
  {
    ROS_INFO_NAMED("descartes", "%s: World frame '%s' does not match model root frame '%s', all poses will be"
...
bool MoveitStateAdapter::getAllIK(const Eigen::Affine3d& pose, std::vector<std::vector<double> >& joint_poses) const
{
  ...
  ROS_DEBUG_NAMED("descartes", "Utilizing an min. difference of %f between IK solutions", epsilon);
  joint_poses.clear();
  for (size_t sample_iter = 0; sample_iter < seed_states_.size(); ++sample_iter)
  {
    ...
    if (getIK(pose, joint_pose))
    {
      if (joint_poses.empty())
      {
        ...
        ROS_DEBUG_NAMED("descartes", msg.str().c_str());
      ...
      else
      {
        ...
        for (joint_pose_it = joint_poses.begin(); joint_pose_it != joint_poses.end(); ++joint_pose_it)
        {
          ...
          {
            ROS_DEBUG_NAMED("descartes", "Found matching, potential solution is not new");
            match_found = true;
            break;
          }
        }
  ...
  ROS_DEBUG_NAMED("descartes", "Found %lu joint solutions out of %lu iterations", static_cast<unsigned long>(joint_poses.size()),
           static_cast<unsigned long>(seed_states_.size()));

  if (joint_poses.empty())
  {
    ROS_ERROR_NAMED("descartes", "Found 0 joint solutions out of %lu iterations", static_cast<unsigned long>(seed_states_.size()));
    return false;
  }
  else
  {
    ROS_INFO_NAMED("descartes", "Found %lu joint solutions out of %lu iterations", static_cast<unsigned long>(joint_poses.size()),
              static_cast<unsigned long>(seed_states_.size()));
    return true;
  }
}
...
bool MoveitStateAdapter::getFK(const std::vector<double>& joint_pose, Eigen::Affine3d& pose) const
{
  ...
  if (isValid(joint_pose))
  {
    ...
    {
      ROS_ERROR_NAMED("descartes", "Robot state does not recognize tool frame: %s", tool_frame_.c_str());
      rtn = false;
    }
  }
  else
  {
    ROS_ERROR_NAMED("descartes", "Invalid joint pose passed to get forward kinematics");
    rtn = false;
  }
...
bool MoveitStateAdapter::isValid(const std::vector<double>& joint_pose) const
{
  ...
  {
    ROS_ERROR_NAMED("descartes", "Size of joint pose: %lu doesn't match robot state variable size: %lu",
